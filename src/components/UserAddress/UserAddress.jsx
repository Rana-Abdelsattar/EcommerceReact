...
let {onlinePayment,getLoggedUserCart,setNumbrtOfItem}=useContext(cartContext)
   ...
   async function AddressSubmit(values)
    {
        ...
        let response=await onlinePayment(data.data._id,'http://localhost:3000',values);
        setNumbrtOfItem()
 ...
 onSubmit:AddressSubmit
  ...
  return (
    <div>
       ...
       <button type='submit' onClick={()=>AddressSubmit()} className='btn bg-main text-light mt-3' >Pay Now</button>
       </form>
